...
<div class="admin-box container">
    <div class="row">
        <div class="col-md-4">
            <p>
                <strong><?php echo lang('emailer_total_in_queue'); ?></strong> <?php echo $total_in_queue ? $total_in_queue : '0'; ?>
            </p>
        </div>
        <div class="col-md-4">
            <p><strong><?php echo lang('emailer_total_sent'); ?></strong> <?php echo $total_sent ? $total_sent : '0'; ?>
            </p>
        </div>
        <div class="col-md-4 text-right">
            <?php echo form_open($this->uri->uri_string(), array('class' => 'form-inline')); ?>
            <input type="submit" name="force_process" class="btn btn-primary"
                   value="<?php e(lang('emailer_force_process')); ?>"/>
            <input type="submit" name="insert_test" class="btn btn-warning"
                   value="<?php e(lang('emailer_insert_test')); ?>"/>
            <?php echo form_close(); ?>
        </div>
    </div>
    <?php if (empty($emails) || !is_array($emails)) : ?>
        ...
        <table class="table table-striped">
            ...
            <?php if ($this->pagination->create_links()) : ?>
                <tr>
                    <td colspan="<?php echo $numColumns; ?>"
                        class="text-left"><?php echo $this->pagination->create_links(); ?></td>
                </tr>
            <?php endif; ?>
            </tfoot>
            <tbody>
            <?php foreach ($emails as $email) : ?>
                <tr>
                    <td class='column-check'><input type="checkbox" name="checked[]" value="<?php echo $email->id; ?>"/>
                    </td>
